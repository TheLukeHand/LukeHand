...
int numWindows(const arma::mat& matrix, int fs, int win_size, int win_disp) {
	...
	arma::colvec armvec = arma::conv_to< arma::colvec >::from(svec);
	arma::uvec idx = find(armvec <= mat_size + 1);
	return idx.n_rows;

}

//Calculate Line length feature of a matrix given a raw data matrixe
arma::mat lineLength(const arma::mat& X) {
	return arma::sum(arma::abs(arma::diff(X)));

}

//Calculate Area Feature Matrix given raw data
arma::mat area(const arma::mat& X) {
	return sum(abs(X));
}

//Calculate Log variance matrix

arma::mat logVar(const arma::mat& X) {
	//cout << "Calculate logvar" << endl;
	//Sleep(2000);
	arma::mat log_variance = log10(var(X, 0, 1));
	//cout << log_variance(0, 0) << endl;
	return arma::sum(log_variance);
}

//Claculates Feature Matrix for a given raw data matrix
arma::mat calculateFeatures(const arma::mat& raw_data, int num_windows, int win_size, int win_disp, int fs) {
	arma::mat ll_mat;
	arma::mat area_mat;
	arma::mat log_var;
	...
	for (int i = 1; i <= num_windows; ++i) {
		...
		//cout << p << endl << q << endl;
		//Sleep(2000);
		arma::mat clip = raw_data.rows(p - 1, q - 1);
		ll_mat.row(i - 1) = lineLength(clip);
		area_mat.row(i - 1) = area(clip);
		log_var.row(i - 1) = logVar(clip);
	}
	//add elements horizontally:
	arma::colvec ll_vec = sum(ll_mat, 1);
	arma::colvec area_vec = sum(area_mat, 1);
	arma::colvec logvar_vec = sum(log_var, 1);


	arma::mat feature_mat;
	feature_mat = join_horiz(ll_vec, join_horiz(area_vec, log_var));
	return feature_mat;
}

cv::Mat arma2CV(arma::mat arma_mat) {
...
Poco::Net::IPAddress get_localhost_address() {
	Poco::Net::IPAddress ipv4_address;
	...
	try {
		...
		for (const auto& address : entry.addresses()) {
			const bool is_ipv4 = address.family() == Poco::Net::IPAddress::IPv4;
		...
		ipv4_address = Poco::Net::IPAddress("127.0.0.1");
	}
	return ipv4_address;
}
...
int main(int /*argc*/, char** /*argv*/)
{
	...
	arma::mat rest, fist, wavein, waveout, thumbsup;
	arma::mat rest_feats, fist_feats, wavein_feats, waveout_feats, thumbsup_feats;
	cout << "Loading Files ..." << endl;
	rest.load("rest.csv", arma::csv_ascii);
	fist.load("fist.csv", arma::csv_ascii);
	wavein.load("wavein.csv", arma::csv_ascii);
	waveout.load("waveout.csv", arma::csv_ascii);
	thumbsup.load("thumbsup.csv", arma::csv_ascii);
	...
	*/
	//create training set:
	arma::mat rest_train, fist_train, waveout_train, wavein_train, thumbsup_train;
	arma::mat rest_test, fist_test, waveout_test, wavein_test, thumbsup_test;
	arma::mat rest_trainlabels, fist_trainlabels, waveout_trainlabels, wavein_trainlabels, thumbsup_trainlabels;
	arma::mat rest_testlabels, fist_testlabels, waveout_testlabels, wavein_testlabels, thumbsup_testlabels;
	...
	arma::rowvec trainlabels_vec = vectorise(trainlabels, 1);
	arma::rowvec testlabels_vec = vectorise(testlabels, 1);
	...
	stdvec trainlabels_std = arma::conv_to< stdvec >::from(trainlabels_vec);
	stdvec testlabels_std = arma::conv_to< stdvec >::from(testlabels_vec);
	...
	for (int k = 0; k < test_data_mat.rows; ++k) {
		cv::Mat sample = (cv::Mat_<float>(1, 3) << testmat(k, 0), testmat(k, 1), testmat(k, 2));
	...
	ServerManager server(get_localhost_address(), 81);
	...
	while (x != -1) {
		...
		if (sample_count >= num_sample_values + num_attempts*overlap_values) {
			arma::mat raw_window = buffer.rows(num_attempts * overlap_values, num_attempts * overlap_values + num_sample_values);
			arma::mat features = calculateFeatures(raw_window, 1, WIN_SIZE, WIN_DISP, FS);
			...
			server.broadcast_pose((uint8_t)gesture);
			num_attempts++;
